...
Forces first, then supporting terms — alphabetical within each group.
...
Range terms first, then optimizer terms — alphabetical within each group.

| Term | Definition |
|------|-----------|
| **BPS** | Basis points. 1 BPS = 0.01% = 0.0001. Used for slippage (default 50 BPS). Note: fee tiers use a 1,000,000 divisor (0.0001% granularity), giving sub-BPS precision. |
...
Definitions first, then tick-based terms, bin-based terms, and V4-specific.

| Term | Definition |
|------|-----------|
| **CLMM** | Concentrated Liquidity Market Maker. Umbrella term for V3/V4/Algebra AMMs. |
| **DLMM** | Discretized Liquidity Market Maker. Synonym for Liquidity Book (Trader Joe, Meteora style). Uses discrete bins instead of continuous ticks. |
...
| Term | Definition |
|------|-----------|
| **V3** | Standard Uniswap V3 interface (NonfungiblePositionManager). Used by Uni V3, PCS V3, Pharaoh, Ramses, Pangolin, Project X, Aerodrome. Aerodrome is a V3 fork on Base with minor `slot0` ABI differences and ve(3,3) mechanics. |
...
| **LB** | Trader Joe Liquidity Book (DLMM). ERC-1155 bin tokens, `positionId` format: `lb:<lowerBin>:<upperBin>`. |
...
| **Permit2** | Canonical Uniswap approval contract (`0x000...22D473`). Used for gasless token approvals with expiry. Required by V4 pools. See [transactions](execution/transactions.md). |
...
| Term | Definition |
|------|-----------|
| **distributed lock** | DragonflyDB-backed mutex using Lua CAS (compare-and-swap). Ensures single-writer semantics for the orchestrator and per-pair worker locks. See [orchestrator](infrastructure/orchestrator.md). |
...
| **heartbeat** | Periodic liveness signal (10s interval) from each worker to DragonflyDB. The orchestrator's health monitor restarts workers that miss 3 consecutive heartbeats. |
...
| **process topology** | Orchestrator/worker tree: one orchestrator process supervises N worker processes (one per pair). Workers are independent OS processes communicating via DragonflyDB. See [orchestrator](infrastructure/orchestrator.md). |
| **pub/sub** | DragonflyDB publish/subscribe channel (`btr:control`) for real-time commands (pause, resume, rebalance) from the API to workers. |
| **TCP transport** | Persistent TCP socket for OpenObserve ingestion. Avoids per-request connection overhead; auto-reconnects on failure. See [observability](infrastructure/observability.md). |
